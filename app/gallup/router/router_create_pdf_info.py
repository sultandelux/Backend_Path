...
def get_pdf_similarity(
    ...
    df = pd.read_csv(
        "https://galluppublic.s3.eu-north-1.amazonaws.com/posts/30%2Bprofessions%2B(for%2Btesting)+%2B+fields.csv"
    )
    df = df.drop(columns=["Сфера деятельности", "Profession (En)", "Match"])
    ...
    for j in range(len(df.iloc[:, 0])):
        ...
        professions.append(round((Max_error - Total_num) * 100 / Max_error, 2))
...
def get_pdf_comments(
    ...
    if pdf_url.get("pdf_comment"):
        ...
        return content_str
    pdf_url = pdf_url.get("url")
    df = pd.read_csv(pdf_url, header=None)
    ...
    pdf_sim_url = svc.repository.get_pdf_url(pdf_id=pdf_id, user_id=jwt_data.user_id)
    pdf_sim_url = pdf_sim_url.get("pdf_similarities")
    df_prof = pd.read_csv(pdf_sim_url)
    df_prof = df_prof.iloc[0:5, 2].tolist()
    srt_prof = ", ".join(str(x) for x in df_prof)
    Data_str = svc.repository.get_pdf_url(pdf_id=pdf_id, user_id=jwt_data.user_id)
    MBTI_str = Data_str.get("MBTI")
    MIT_str = Data_str.get("MIT")
